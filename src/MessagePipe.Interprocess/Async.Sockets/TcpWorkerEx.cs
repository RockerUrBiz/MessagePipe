    ...
    namespace MessagePipe.Interprocess.Async.Sockets
    {
        ...
        public sealed class TcpWorkerEx : IDisposable
        {
            ...
            public void Publish<TKey, TMessage>(TKey key, TMessage message)
            {
                ...
                {
                    _ = client.Value; // Access the tcpClientEx instance
                    RunPublishLoop();
                }
            ...
            async void RunPublishLoop()
            {
                ...
                var tcpClient = client.Value; // Access the tcpClientEx instance
            ...
            /// </summary>
            /// <param name="client">The tcpClientEx.</param>
            /// <autogeneratedoc />
            /// 
            ...
            private async void RunReceiveLoop(SocketTcpClientEx client)
            {
                ...
                while (!token.IsCancellationRequested)
                {
                    try
                    {
                        ...
                        await ProcessReceivedMessage(client, buffer.AsMemory(0, readLen)).ConfigureAwait(false);
                    }
                    catch (Exception ex)
                    {
            ...
            /// </summary>
            /// <param name="tcpClientEx">The tcpClientEx.</param>
            /// <param name="initialBuffer">The initial buffer.</param>
            /// <autogeneratedoc />
            /// TODO Edit XML Comment Template for ProcessReceivedMessage
            private async Task ProcessReceivedMessage(SocketTcpClientEx tcpClientEx, ReadOnlyMemory<byte> initialBuffer)
            {
                var token = cancellationTokenSource.Token;
                var buffer = new byte[65536];
                ...
                try
                {
                    while (true)
                    {
                        if (readBuffer.Length < 4)
                        {
                            ...
                            readBuffer = await ReadMoreAsync(tcpClientEx, buffer, readBuffer, token).ConfigureAwait(false);
                            continue;
                        }
                        ...
                        await ParseMessage(message, token).ConfigureAwait(false);
...
#endif
                                    await ((System.Threading.Tasks.Task)task!).ConfigureAwait(false); // Task<T> -> Task
            ...
            private void Dispose(bool disposing)
            {
                if (!_disposed)
                {
                    if (disposing)
                    {
                        ...
                        // Dispose the server and tcpClientEx if they have been created
