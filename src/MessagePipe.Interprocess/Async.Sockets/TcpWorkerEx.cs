...
namespace MessagePipe.Interprocess.Async.Sockets
{
    ...
    namespace MessagePipe.Interprocess.Async.Sockets
    {
        ...
        public sealed class TcpWorkerEx : IDisposable
        {
            ...
            //async void RunReceiveLoop(SocketTcpClientEx tcpClientEx)
            //{
            //    var token = cancellationTokenSource.Token;
            ...
            //        try
            //        {
            //            int readLen = await tcpClientEx.ReceiveAsync(buffer, 0, buffer.Length, token).ConfigureAwait(false);
            //            if (readLen == 0)
                ...
                try
                {
                    while (true)
                    {
                        if (readBuffer.Length < 4)
                        {
                            ...
                            readBuffer = await ReadMoreAsync(tcpClientEx, buffer, readBuffer, token);
                            continue;
                        }
