...
using MessagePipe.Interprocess.Async.Sockets;
...
namespace MessagePipe
{
    public static class ServiceCollectionInterprocessExtensions
    {
        ...
        /// <summary>
        /// Adds the message pipe TCP interprocess ex.
        /// </summary>
        ...
        /// <returns></returns>
        /// <autogeneratedoc />
        /// TODO Edit XML Comment Template for AddMessagePipeTcpInterprocessEx
        public static ReturnType AddMessagePipeTcpInterprocessEx(this IServiceCollection services, string host, int port, Action<MessagePipeInterprocessTcpOptionsEx> configure)
        {
            var options = new MessagePipeInterprocessTcpOptionsEx(host, port);
            configure(options);

            services.AddSingleton(options);
            services.Add(typeof(TcpWorkerEx), options.InstanceLifetime);

            // Example values for the parameters - adjust as necessary
            int initialCapacity = 10;
            int maxSize = 100;
            int minSize = 5;
            TimeSpan maxIdleTime = TimeSpan.FromMinutes(5);
            TimeSpan contractionInterval = TimeSpan.FromMinutes(1);

            services.AddSingleton<SocketAsyncEventArgsPool>(sp =>
                                                                new SocketAsyncEventArgsPool(initialCapacity, maxSize, minSize, maxIdleTime, contractionInterval));

            // Rest of the service registrations...
            //services.Add(typeof(SocketAsyncEventArgsPool), typeof(SocketAsyncEventArgsPool), options.InstanceLifetime);

#if !UNITY_2018_3_OR_NEWER
            services.Add(typeof(IDistributedPublisher<,>), typeof(TcpDistributedPublisherEx<,>), options.InstanceLifetime);
            services.Add(typeof(IDistributedSubscriber<,>), typeof(TcpDistributedSubscriberEx<,>), options.InstanceLifetime);
            services.Add(typeof(IRemoteRequestHandler<,>), typeof(TcpRemoteRequestHandlerEx<,>), options.InstanceLifetime);
            return services;
#else
            AddAsyncMessageBroker<IInterprocessKey, IInterprocessValue>(services, options);
            return options;
